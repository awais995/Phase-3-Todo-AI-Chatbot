...
def run_comprehensive_test():
    print("Running Comprehensive Chatbot Functionality Test...")
    print("="*70)
    ...
    if chat_response.status_code == 200:
        response_data = chat_response.json()
        conversation_id = response_data.get('conversation_id')
        print("   [OK] Second task addition request successful")
        print(f"   Response: '{response_data.get('response', '')}'")
    ...
    if chat_response.status_code == 200:
        response_data = chat_response.json()
        print("   [OK] Task update request successful")
        print(f"   Response: '{response_data.get('response', '')}'")
    ...
    if chat_response.status_code == 200:
        response_data = chat_response.json()
        print("   [OK] Task completion request successful")
        print(f"   Response: '{response_data.get('response', '')}'")
    ...
    if chat_response.status_code == 200:
        response_data = chat_response.json()
        print("   [OK] Task deletion request successful")
        print(f"   Response: '{response_data.get('response', '')}'")
    ...
    if chat_response.status_code == 200:
        response_data = chat_response.json()
        print("   [OK] Final task listing request successful")
        print(f"   Response: '{response_data.get('response', '')}'")
    ...
    print("\n" + "="*70)
    print("✅ Comprehensive Chatbot Functionality Test Completed Successfully!")
    print("✅ All operations (Add, List, Update, Complete, Delete) worked correctly")
    print("✅ The chatbot is properly integrated with the backend task management system")
    return True

if __name__ == "__main__":
    success = run_comprehensive_test()
    if success:
        print("\n🎉 All tests passed! The chatbot functionality is working as expected.")
    else:
